...
public final class CliOptions {
    ...
    private static Option helpOption() {
        return Option.builder().option("h").longOpt("help").desc("Print this message").build();
    }

    private static Option problemNumberOption() {
        return Option.builder().option("p").longOpt("problem")
                .desc("The problem to be executed in yy:dd:pp or " + "yyyy:dd:pp format. "
                        + "If omitted, all problems will be solved").hasArg().type(String.class).build();
    }

    private static Option problemOtherNumberOption() {
        return Option.builder().option("pb").longOpt("problem-bound")
    ...
    private static Option debugOption() {
        return Option.builder().option("d").longOpt("debug").desc("Use debug logging").build();
    }
}
