...
public final class D17 {
    ...
    private Computer loadResourcesAsComputer(final ProblemResourceLoader<List<String>> resourceLoader) {
        List<String> lines = resourceLoader.resources();
        int registerA = Integer.parseInt(lines.get(0).trim().replaceAll("\\s+", "").replaceAll("RegisterA:", ""));
        int registerB = Integer.parseInt(lines.get(1).trim().replaceAll("\\s+", "").replaceAll("RegisterB:", ""));
        int registerC = Integer.parseInt(lines.get(2).trim().replaceAll("\\s+", "").replaceAll("RegisterC:", ""));

        String[] programStrs = lines.get(4).replaceAll("\\s+", "").replaceAll("Program:", "").split(",");
    ...
    private static final class InputChecker {
        ...
        private static TreeSet<Pair<Integer, Integer>> computeThreeBitXorSolutions(int target) {
            TreeSet<Pair<Integer, Integer>> solutions = new TreeSet<>(Comparator.comparingInt(i -> (i.getLeft() ^ 5)));
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    if ((i ^ j) == target) {
        ...
        public void check(final int prgIndex, long regA, long checkedBits) {
            ...
            for (Pair<Integer, Integer> solutionPair : solutionPairs) {
                ...
                regAcpy |= ((long) c << cShift);
                checkedBitsCpy |= (7L << cShift);
        ...
        private boolean check3BitsMatch(long checked, long proposedValue, long value) {
            for (int checkFlag : CHECK_FLAGS) {
                if ((checked & checkFlag) != 0 && (proposedValue & checkFlag) != (value & checkFlag)) {
                    return false;
                }
            }
            return true;
    ...
    private static final class Computer {
        ...
        private static final ColumnFormatter<Computer>[] COMPUTER_COLS = new ColumnFormatter[]{
                new ColumnFormatter.FuncColumnFormatter<Computer>("SP", 20, ' ',
                        comp -> Integer.toString(comp.stackPointer)),
                new ColumnFormatter.FuncColumnFormatter<Computer>("INS", 20, ' ',
                        comp -> Integer.toString(comp.program[comp.stackPointer])),
                new ColumnFormatter.FuncColumnFormatter<Computer>("OP", 20, ' ',
                        comp -> Integer.toString(comp.program[comp.stackPointer + 1])),
                new ColumnFormatter.FuncColumnFormatter<Computer>("COMBO", 20, ' ',
                        comp -> Long.toString(comp.comboOperand(comp.program[comp.stackPointer + 1]))),
                new ColumnFormatter.FuncColumnFormatter<Computer>("A", 20, ' ', comp -> Long.toString(comp.registerA)),
                new ColumnFormatter.FuncColumnFormatter<Computer>("B", 20, ' ', comp -> Long.toString(comp.registerB)),
                new ColumnFormatter.FuncColumnFormatter<Computer>("C", 20, ' ', comp -> Long.toString(comp.registerC))};
        private final int[] program;
        private final Printer<Computer> printer = new Printer<>(COMPUTER_COLS);
        ...
        private Computer(int registerA, int registerB, int registerC, final int[] program) {
        ...
        private void execute(final int opCode, final int operand) {
            if (opCode == ADV) {
                registerA = registerA / (1L << comboOperand(operand));
                stackPointer += 2;
                return;
            }
            if (opCode == BXL) {
            ...
            if (opCode == BDV) {
                registerB = (registerA / (1L << comboOperand(operand))) % 8;
                stackPointer += 2;
                return;
            }
            if (opCode == CDV) {
                registerC = (registerA / (1L << comboOperand(operand)) % 8);
                stackPointer += 2;
            }
        }
