...
public final class D24 {
    ...
    public Long partOneImpl(final ProblemResourceLoader<List<String>> resourceLoader) {
        ...
        final SetUtils.Constraint<Long> constraint =
                new SetUtils.Constraint<>((i, arr) -> ArrayUtils.sum(arr, i) > target,
                        (i, arr) -> ArrayUtils.sum(arr, i) == target);
        List<Long[]> candidates =
                SetUtils.constrainedSetsOfSizeLeqN(packages, Arrays.copyOf(packages, packages.length / 3), constraint);

        candidates.sort(Comparator.comparingInt(a -> a.length));
        final int searchEnd = BinarySolutionSearch.search(0, candidates.size(),
        ...
        for (int i = 0; i < searchEnd; i++) {
            ...
            List<Long[]> middles =
                    SetUtils.constrainedSetsOfSizeLeqN(remaining, Arrays.copyOf(remaining, remaining.length / 2),
                            constraint);
    ...
    public Long partTwoImpl(final ProblemResourceLoader<List<String>> resourceLoader) {
        ...
        final SetUtils.Constraint<Long> constraint =
                new SetUtils.Constraint<>((i, arr) -> ArrayUtils.sum(arr, i) > target,
                        (i, arr) -> ArrayUtils.sum(arr, i) == target);
        List<Long[]> candidates =
                SetUtils.constrainedSetsOfSizeLeqN(packages, Arrays.copyOf(packages, packages.length / 4), constraint);
        ...
        for (int i = 0; i < searchEnd; i++) {
            ...
            Long[] remaining = SetUtils.copyRemoveAll(packages, candidates.get(i));
            List<Long[]> leftMiddles =
                    SetUtils.constrainedSetsOfSizeLeqN(remaining, Arrays.copyOf(remaining, remaining.length / 3),
                            constraint);
            for (int j = 0; j < leftMiddles.size(); j++) {
                Long[] remainingLeft = SetUtils.copyRemoveAll(remaining, leftMiddles.get(j));
                List<Long[]> rightMiddles = SetUtils.constrainedSetsOfSizeLeqN(remainingLeft,
                        Arrays.copyOf(remainingLeft, remaining.length / 2), constraint);
    ...
    private Long[] loadPackages(final ProblemResourceLoader<List<String>> resourceLoader) {
