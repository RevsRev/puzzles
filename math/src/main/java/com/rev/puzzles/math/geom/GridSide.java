...
public final class GridSide {
    ...
    public boolean equals(final Object o) {
        ...
        final GridSide gridSide = (GridSide) o;
        return Objects.equals(start, gridSide.start) && Objects.equals(end, gridSide.end)
                && direction == gridSide.direction;
    }
    ...
    public String toString() {
        return "GridSide{" + "start=" + start + ", end=" + end + ", direction=" + direction + '}';
    }

    private static IntersectionResult pointIntersect(final GridSide first, final GridSide second) {
        final Mat2 mat = new Mat2((first.start.x() - first.end.x()), -(second.start.x() - second.end.x()),
        ...
        final Vec2 thisGradient = new Vec2(first.start.x() - first.end.x(), first.start.y() - first.end.y());
        final Vec2 intersection =
                new Vec2(first.start.x(), first.start.y()).plus(thisGradient.mult(intersectionParams.getX()));

        if (withinEpsilonOfInteger(intersection.getX()) && withinEpsilonOfInteger(intersection.getY())) {
            final GridPoint maybeIntersection =
                    new GridPoint(Math.round(intersection.getX()), Math.round(intersection.getY()));
            if (first.contains(maybeIntersection) && second.contains(maybeIntersection)) {
                return new PointIntersectionResult(maybeIntersection);
            }
        }
        return new EmptyIntersectionResult();
    }
    ...
    private static IntersectionResult parallelIntersect(final GridSide first, final GridSide second) {
        if ((first.direction.equals(LEFT) || first.direction.equals(RIGHT)) && first.start.y() == second.start.y()) {
            ...
            if (start.equals(end)) {
                ...
                return new PointSideIntersectionResult(withOrientation(GridSide.create(start, end), first.direction));
            }
        } else if ((first.direction.equals(UP) || first.direction.equals(DOWN))
